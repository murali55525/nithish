...
DB_CONFIG = {
    ...
    "host": os.getenv("DB_HOST", "database-service"),  # ✅ FIX
...
def get_patients():
    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT id, name, age, condition, risk FROM patients ORDER BY name")
    data = cur.fetchall()
    cur.close()
    conn.close()
    return jsonify(data)
...
def update_patient(id):
    ...
    try:
        ...
        params.append(id)
        query = f"UPDATE patients SET {', '.join(updates)} WHERE id = %s RETURNING id, name, age, condition, risk"
...
def create_appointment():
    ...
    try:
        cur.execute("""
            INSERT INTO appointments (patient_id, patient_name, date_time, doctor, status)
            VALUES (%s, %s, %s, %s, %s)
            RETURNING id, patient_id, patient_name, date_time, doctor, status
        """, (data["patient_id"], data["patient_name"], data["date_time"], data["doctor"], data["status"]))
...
def update_appointment(id):
    ...
    try:
        ...
        params.append(id)
        query = f"UPDATE appointments SET {', '.join(updates)} WHERE id = %s RETURNING id, patient_id, patient_name, date_time, doctor, status"
        cur.execute(query, params)
        updated = cur.fetchone()
        if not updated:
            conn.rollback()
